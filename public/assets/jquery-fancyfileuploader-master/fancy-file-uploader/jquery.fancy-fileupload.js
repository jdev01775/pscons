	...
	$.fn.FancyFileUpload = function(options) {
		...
		var AddFile = function(uploads, e, data) {
			var inforow = $(`<div class="mt-3">
								...
								<div class="ff_fileupload_summary d-none">
								...
								<div class="ff_fileupload_actions"></div>
			...
			if (URL && URL.createObjectURL)
			{
				...
				if (url)
				{
					if (data.files[0].type === 'image/gif' || data.files[0].type === 'image/jpeg' || data.files[0].type === 'image/png')
					{
						...
						inforow.find('.ff_fileupload_preview_image').attr(`data-src`,url)
						inforow.find('.ff_fileupload_preview_image').attr(`data-file-name`,data.files[0].name)
						haspreview = true;
		...
		return this.each(function() {
			...
			dropzone.html(`<span>ลากไฟล์รูปมาใส่ที่นี้ หรือ กด</span>
							<button type="button" class="btn btn-upload"> 
								<img class="align-self-center" src="${rootUrl}/img/icon_camera1.svg"/>
			...
			var uploads = $('<div>').addClass('ff_fileupload_uploads');
