...
<div class="setting-header">
    <span class="setting-title text-bold">แก้ไขผู้ใช้งาน</span>
</div>
     ...
     <div class="user-profile-img-contrainer">
         <div class="profile-img">
            <div class="circle">
                ...
                @if ( count($data_users_link_img) > 0)
                {{asset($data_users_link_img[0]['users_link_img_path'])}}
                @else
                {{ asset('img/profile_img.svg') }}   
                @endif
         ...
         @if ($data_users[0]['id'] != Auth::user()->id)
         <div class="profile-delete">
            <button   type="button" class="btn btn-profile-delete" id="remove_users" data-users-id="{{$data_users[0]['id']}}">
                ลบข้อมูล
            </button>
        </div>
         @endif
     ...
     <div class="user-profile-detail-contrainer">
            <div class="row-detail">
                <div class="w-50 mt-1">
                    ...
                    <input class="form-control form-control-custom" type="text" placeholder="โปรดระบุ" name="user_firstname" value="{{$data_users[0]['user_firstname']}}">
                </div>
                <div class="w-50 mt-1">
                   <span class="text-bold">นามสกุล</span>
                   <input class="form-control form-control-custom" type="text" placeholder="โปรดระบุ" name="user_lastname" value="{{$data_users[0]['user_lastname']}}">
               </div>
            </div>  
            <div class="row-detail">
                <div class="w-50 mt-1">
                    <span class="text-bold">Email (ใช้สำหรับการเข้าสู่งระบบ และ การแจ้งเตือนต่างๆ)</span>
                    <input class="form-control form-control-custom" type="text" placeholder="โปรดระบุ" name="email" value="{{$data_users[0]['email']}}"> 
                </div>
                <div class="w-50 mt-1">
                    ...
                    <select class="form-control form-control-custom " title="โปรดระบุ" name="position_id">
                        ...
                        @foreach ($data_position as $item)
                            ...
                            @if ($data_users[0]['position_id'] == $item['position_id'])
                                selected
                            @endif
             ...
             <div class="row-detail-2">
                ...
                <label class="container-checkbox pl-25px"> 
                    ...
                    @if (count($data_projects) == count($projects_id_list))
                        checked
                    @endif
                    >
             ...
             <div class="project-list-container">
                 ...
                 <div class="project-checkbox-layout">
                    <label class="container-checkbox pl-25px"> 
                        ...
                        @if (in_array($item['projects_id'], $projects_id_list))
                            checked
                        @endif
                        >
                        <span class="checkmark"></span>
...
const save_data = ()=>{
        ...
        let project_link_users = $(`[name^=permission_id]:checked`).map(function(){
            ...
            data.append('user_id',`{{$data_users[0]['id']}}`);
            
        Swal.fire({
            imageUrl: `{{ asset('/img/icon_alert_question.svg') }}`,
            html:`<span>คุณต้องการแก้ไขผู้ใช้งาน ?</span>`,
            showDenyButton: true,
            allowOutsideClick: false,
            confirmButtonText,
            denyButtonText,
            ...
            if(res.isConfirmed){
                ...
                $.ajax({
                    type: "POST",
                    url: `{{url('setting/setting_user_update')}}`,
                    data,
                    cache:false,
                    contentType: false,
                    processData: false,
                    ...
                    success: function (response) {
                        Swal.fire({
                        imageUrl: `{{ asset('/img/icon_alert_success.svg') }}`,
                        html:`<span>คุณได้ทำการแก้ไขผู้ใช้งานเรียบร้อยแล้ว</span>`,
                        showDenyButton: false,
                        allowOutsideClick: false,
                        confirmButtonText: `ตกลง`,
                        denyButtonText: `ยกเลิก`,
